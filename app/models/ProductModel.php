...
class ProductModel extends Database {
    ...
    // [FIX LỖI CÚ PHÁP ĐƯỜNG TRUYỀN DATABASE]
    public function getGalleryImages($productId) {
        $db = $this->getConnection();
        $stmt = $db->prepare("SELECT * FROM product_images WHERE product_id = ? AND is_main = 0");
    ...
    public function deleteProduct($id) {
        ...
        $db->begin_transaction();
        try {
            $db->query("DELETE FROM cart_items WHERE product_id = $id"); 
