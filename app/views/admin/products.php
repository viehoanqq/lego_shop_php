<style>
    ...
    * { box-sizing: border-box; }
    ...
    .form-control { padding: 10px; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 14px; transition: 0.2s; }
    ...
    /* CSS CHO Ô UPLOAD ẢNH MỚI */
    .image-upload-box {
        width: 140px; height: 140px; border: 2px dashed #cbd5e1; border-radius: 12px;
        display: flex; align-items: center; justify-content: center; flex-direction: column;
        cursor: pointer; overflow: hidden; position: relative; background: #f8fafc; transition: 0.2s;
    }
    ...
    .image-upload-box:hover .image-upload-placeholder i { color: #3182ce; transform: translateY(-3px);}
</style>
                ...
                <span>
                    <?php
                        switch($session_error) {
                            ...
                            case 'has_history':   echo "Không thể xóa! Sản phẩm đã có trong đơn hàng/phiếu nhập."; break;
                            case 'hidden_due_to_history': echo "Sản phẩm có lịch sử nên đã được tự động Khóa thay vì Xóa!"; break;
                            case 'cat_is_locked': echo "Danh mục của sản phẩm này đã bị khóa, không thể mở bán."; break;
            ...
            <tbody>
                <?php if(!empty($products)): ?>
                    ...
                    <tr>
                            ...
                            <div class="product-cell">
                                ...
                                <div>
                                    <a href="/lego_shop_php/adminproduct/detail/<?= $p['id'] ?>" style="text-decoration: none; display: block;">
                                        <div style="font-weight: 700; color: #3182ce; transition: color 0.2s;" onmouseover="this.style.color='#2c5282'" onmouseout="this.style.color='#3182ce'">
                        ...
                        <td style="font-size: 13px; font-weight: 500; color: #4a5568;"><span style="background: #edf2f7; padding: 3px 8px; border-radius: 4px;"><?= htmlspecialchars($p['category_name']) ?></span></td>
                        <td class="price-tag"><?= number_format($p['selling_price'], 0, ',', '.') ?>đ</td>
                        <td style="font-size: 13px; color: #718096;"><strong><?= number_format($p['pieces']) ?></strong> pcs</td>
                        <td>
                        ...
                        <td>
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                ...
                                <?php 
                                    ...
                                    $btnColor = $isActive ? '#e53e3e' : '#38a169'; 
                                ...
                                <a href="/lego_shop_php/adminproduct/toggleStatus/<?= $p['id'] ?>?current=<?= $p['status'] ?>" 
                                    ...
                                    <span style="margin-left: 4px; font-weight: 600;"><?= $btnText ?></span>
                                </a>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="text-align: center; padding: 40px; color: #a0aec0;">Chưa có sản phẩm nào.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<?php else: ?>
    ...
    <div class="form-container">
        <h3 style="margin-top:0; color: #1e293b;">
            <?= isset($product) ? '<i class="fa-solid fa-pen-to-square" style="color: #f59e0b;"></i> Chỉnh sửa: ' . htmlspecialchars($product['name']) : '<i class="fa-solid fa-folder-plus" style="color: #38a169;"></i> Thêm sản phẩm mới' ?>
        </h3>
        ...
        <form id="productForm" action="<?= isset($product) ? '/lego_shop_php/adminproduct/update/'.$product['id'] : '/lego_shop_php/adminproduct/store' ?>" method="POST" enctype="multipart/form-data">
            ...
            <div style="background: #f8fafc; padding: 20px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 20px;">
                <h4 style="margin: 0 0 15px 0; color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px;">1. Thông tin cơ bản</h4>
                
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 25px;">
                    <div>
                        <div class="form-group">
                            <label>Tên sản phẩm LEGO <span style="color:red">*</span></label>
                            <input type="text" id="name" name="name" class="form-control" value="<?= htmlspecialchars($product['name'] ?? '') ?>">
                            ...
                            <div class="image-upload-box" onclick="document.getElementById('main_image').click()">
                                <?php if(!empty($product['main_image'])): ?>
                                    ...
                                    <div class="image-upload-placeholder" id="image_placeholder" style="display:none;">
                                        <i class="fa-solid fa-cloud-arrow-up"></i><br>Chọn ảnh
                                    </div>
                                <?php else: ?>
                                    <img id="image_preview" src="" style="display:none;">
                                    <div class="image-upload-placeholder" id="image_placeholder">
                                        <i class="fa-solid fa-cloud-arrow-up"></i><br>Nhấn để chọn
                                    </div>
                                <?php endif; ?>
                            </div>
                            <small class="error-text"></small>
            ...
            <div style="background: #fffbeb; padding: 20px; border-radius: 8px; border: 1px solid #fef08a; margin-bottom: 25px;">
                <h4 style="margin: 0 0 15px 0; color: #92400e; border-bottom: 2px solid #fde68a; padding-bottom: 10px;">2. Chi tiết kỹ thuật</h4>
                
                <?php 
                    // Xử lý tách chuỗi kích thước cũ ra 3 ô
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;">
                    <div class="form-group">
                        <label>Số mảnh ghép (Pieces) <span style="color:red">*</span></label>
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 5px;">
                            <input type="number" name="length" class="form-control" placeholder="Dài" value="<?= $l ?>" style="padding: 10px 5px; text-align: center;">
                            <input type="number" name="width" class="form-control" placeholder="Rộng" value="<?= $w ?>" style="padding: 10px 5px; text-align: center;">
                            <input type="number" name="height" class="form-control" placeholder="Cao" value="<?= $h ?>" style="padding: 10px 5px; text-align: center;">
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; gap: 12px;">
                ...
                <a href="/lego_shop_php/adminproduct" style="padding: 10px 20px; color: #475569; text-decoration: none; font-weight: 600; background: #edf2f7; border-radius: 8px;">Hủy bỏ</a>
            </div>
        </form>
    </div>
...
<script>
    ...
    if(form) {
        ...
        function validateName() {
            ...
            if (value === "") { showError(nameInput, "Tên không được để trống!"); return false; }
            if (value.length < 3) { showError(nameInput, "Tên quá ngắn (Tối thiểu 3 ký tự)!"); return false; }
        ...
        function validateSku() {
            ...
            const regex = /^[A-Z0-9\-]+$/;
            if (value === "") { showError(skuInput, "Mã SKU không được để trống!"); return false; }
            if (!regex.test(value)) { showError(skuInput, "SKU chỉ được chứa chữ IN HOA, số và dấu gạch ngang (-)"); return false; }
            showSuccess(skuInput); return true;
        }

        function validatePrice() {
            if (priceInput.hasAttribute('readonly')) {
                showSuccess(priceInput); return true;
            }
            const value = parseInt(priceInput.value);
            if (isNaN(value) || value < 0) { showError(priceInput, "Giá bán không hợp lệ (Phải ≥ 0đ)"); return false; }
            showSuccess(priceInput); return true;
        }

        function validatePieces() {
            const value = parseInt(piecesInput.value);
            if (isNaN(value) || value < 0) { showError(piecesInput, "Số mảnh ghép phải ≥ 0"); return false; }
            showSuccess(piecesInput); return true;
        }

        function validateYear() {
            const value = yearInput.value.trim();
            if (value === "") { showSuccess(yearInput); return true; } 
            const year = parseInt(value);
            const currentYear = new Date().getFullYear();
            if (isNaN(year) || year < 1958 || year > currentYear + 2) { 
                showError(yearInput, `Năm phát hành phải từ 1958 đến ${currentYear + 2}`); 
                return false; 
            }
            showSuccess(yearInput); return true;
        }
        ...
        form.addEventListener("submit", function(e) {
            ...
            const isPiecesValid = validatePieces();
            const isYearValid = validateYear();

            if (!(isNameValid && isSkuValid && isPriceValid && isPiecesValid && isYearValid)) {
                e.preventDefault(); 
                alert("⚠️ Vui lòng kiểm tra lại các trường dữ liệu bị báo đỏ trước khi lưu!");
    ...
    // Hàm preview ảnh ngay khi chọn file
